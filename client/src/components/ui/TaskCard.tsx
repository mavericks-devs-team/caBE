...
import { Clock, BarChart, ChevronRight, CheckCircle2, FileText, Crosshair, Lock } from "lucide-react";
...
export function TaskCard({ task, isCompleted, isHero }: TaskCardProps) {
  ...
  const CardContent = (
    <motion.div
      ...
      onClick={() => {
        if (!user) {
          toast({ title: "Authentication Required", description: "Please login to access task details.", variant: "destructive" });
        }
      }}
    >
      {/* Status Overlay */}
      ...
      <div className="flex flex-col flex-1 h-full pt-4">
        ...
        <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
          ...
          {user ? (
            ...
            <div className="flex items-center gap-2 text-xs font-bold text-muted-foreground opacity-60 group-hover:opacity-100 transition-opacity">
              <Lock className="w-3.5 h-3.5" />
              <span className="uppercase tracking-wider">Login to Inspect</span>
            </div>
          )}
        </div>
      </div>
